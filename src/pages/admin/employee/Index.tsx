...
const EmployeeIndex = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false);
      ...
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/employee`);
          if (response.status === 200 && response.data.success) {
            const formattedData = response.data.data.employees.map((employee: any) => ({
                id: employee._id,
                name: employee.name,
                gender: employee.gender,
                image: employee.image || "",
                address: `${employee.address.houseNo}, ${employee.address.addressLine}, ${employee.address.city}, ${employee.address.province}, ${employee.address.country}`,
                phoneNo: employee.mobileNo1,
            }));
    ...
    const handleAction = async (action: string, id: string) => {
      if (action === 'delete') {
        ...
        try {
          const response = await axios.delete(`${import.meta.env.VITE_API_URL}/employee/${id}`);
          if (response.status === 200 && response.data.success) {
  ...
  const columns = [
      ...
      { name: 'Name', selector: (row: Employee) => row.name, sortable: true, wrap: true },
      { name: 'Gender', selector: (row: Employee) => row.gender, sortable: true, width: '100px' },
      { name: 'Address', selector: (row: Employee) => row.address, sortable: true },
  ...
  return (
    <div className="relative">
      ...
      {/* Pass dynamic title */}
      <TableLayout columns={columns} data={employeeData} onAction={handleAction} />

      {/* Modal for Adding Employee */}
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Add New Employee" size="4xl">
        <AddEmployee
          onSuccess={() => {
          handleCloseModal();
          fetchEmployeeData();
