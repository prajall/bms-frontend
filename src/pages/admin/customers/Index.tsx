...
const CustomerIndex = () => {
      ...
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/customer`);
          if (response.status === 200 && response.data.success) {
            ...
            const formattedData = response.data.data.customers.map((customer: any) => ({
                id: customer._id,
                name: customer.name,
  ...
  const columns = [
      ...
      { name: 'Name', selector: (row: Customer) => row.name, sortable: true, wrap: true },
      { name: 'Gender', selector: (row: Customer) => row.gender, sortable: true, width: '100px' },
      { name: 'Address', selector: (row: Customer) => row.address, sortable: true },
  ...
  return (
    <div className="relative">
      ...
      {/* Pass dynamic title */}
      <TableLayout columns={columns} data={customerData} onAction={handleAction} />

      {/* Modal for Adding Customer */}
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Add New Customer" size="4xl">
        <AddCustomer
          onSuccess={() => {
          handleCloseModal();
          fetchCustomerData();
