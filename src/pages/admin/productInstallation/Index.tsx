...
const ProductInstallationIndex = () => {
  const [productInstallationData, setProductInstallationData] = useState<ProductInstallation[]>([]);
  const [errorMessage, setErrorMessage] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);

    const fetchProductInstallationData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/product-installations`);
          if (response.status === 200 && response.data.success) {
              console.log(response.data.data);
            const formattedData = response.data.data.productInstallations.map((customer: any) => ({
               
            }));
          setProductInstallationData(formattedData);
          setErrorMessage("");
        } else {
          setErrorMessage(response.data.message || "Failed to fetch product Installations.");
        }
      } catch (error) {
    ...
    const handleAction = async (action: string, id: string) => {
      if (action === 'delete') {
        const confirmDelete = window.confirm('Are you sure you want to delete this product Installation?');
        if (!confirmDelete) return;

        try {
          const response = await axios.delete(`${import.meta.env.VITE_API_URL}/product-installations/${id}`);
          if (response.status === 200 && response.data.success) {
          ...
          if (error.response && error.response.data) {
                const errorMessage = error.response.data.message || "Failed to delete product Installation.";
  ...
  const columns = [
      ...
      { name: 'Name', selector: (row: ProductInstallation) => row.name, sortable: true, wrap: true },
      { name: 'Gender', selector: (row: ProductInstallation) => row.gender, sortable: true, width: '100px' },
      { name: 'Address', selector: (row: ProductInstallation) => row.address, sortable: true },
  ...
  return (
    <div className="relative">
      ...
      {/* Pass dynamic title */}
      <TableLayout columns={columns} data={productInstallationData} onAction={handleAction} />

      {/* Modal for Adding ProductInstallation */}
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Create Installation" size="4xl">
