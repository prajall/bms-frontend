...
const ProductCategoryIndex = () => {
    ...
    const fetchCategoryData = async () => {
      try {
        ...
        if (response.status === 200 && response.data.success) {
          const formattedData = response.data.data.categories.map((category: any) => ({
            id: category._id,
            name: category.name,
  ...
  const columns = [
    ...
    {
      name: 'Image',
      selector: (row: Category) => <img src={row.image} alt={row.name} width="50" height="50" />,
      sortable: false,
      width: '90px',
    },
    { name: 'Name', selector: (row: Category) => row.name, sortable: true, wrap: true },
    { name: 'Description', selector: (row: Category) => row.description, sortable: true },
  ...
  return (
    <div className="relative">
      ...
      {/* Pass dynamic title */}
      <TableLayout columns={columns} data={categoryData} onAction={handleAction} />

      {/* Modal for Adding Category */}
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Add New Category" size="xl">
        <AddCategogy
          onSuccess={() => {
          handleCloseModal();
          fetchCategoryData();
